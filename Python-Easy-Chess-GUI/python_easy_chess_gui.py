...
class EasyChessGui:
    ...
    def __init__(self, theme, engine_config_file, user_config_file,
        ...
        self.is_save_user_comment = True########REsemblyzer
        Default_voice = preprocess_wav('./rec2.wav')
        encoder = VoiceEncoder()
        self.default_voice = encoder.embed_utterance(Default_voice)
        ...
        while not board.is_game_over(claim_draw=True):
            ...
            if is_human_stm:
                ...
                while True:
                    ...
                    if type(button) is tuple:                           #Edit Here
                        ...
                        with sr.Microphone() as source:
                            ...
                            try:
                                ...
                                print('Recognising....')
                                #window['-listen-'].update('Recognising....')
                                with open('move.wav', 'wb')as f:
                                ...
                                #window['-listen-'].update('Did not understand....')
                            #valid_input = False
                                continue
                        with sr.AudioFile('./move.wav') as source:
                            try:
                                ...
                                os.remove("Output.mp3")
                                #window['-listen-'].update('Did not understand....')
                        #window['-listen-'].update(value = '')
                        checker = ['0', '1', '2', '3', '4', '5', '6', '7']
                        ...
                        for element in text:
                            if element not in checker:
                                ...
                                print("Invalid Input....")
                                break
                        if len(text) !=4 and valid_input:
                            valid_input = False
                            print("Invalid Input.....")
                        z = preprocess_wav('./move.wav')
                        encoder = VoiceEncoder()
                        player = encoder.embed_utterance(z)
                        similarity = np.inner(player, self.default_voice)
                        if(similarity < 0.7):
                            print('Not a valid user')
                            valid_input = False
                        if valid_input:
                            move_from = [int(text[1]), int(text[0])]
                            move_to = [int(text[3]), int(text[2])]
                        else:
                            move_from = [0, 0]
                            move_to = [0, 0]
                        ...
                        if move_state == 1:
                            ...
                            if user_move in board.legal_moves:
                                ...
                                window['-listen-'].update('Instruction(for Get Voice): Say four one digit nos.')
                            ...
                            # Else if move is illegal
                                ...
                                window['-listen-'].update('Invalid Move!')
    ...
    def build_main_layout(self, is_user_white=True):
        ...
        for i, row in enumerate(board_layout):###############################################################
            row.append(sg.Text(str(i)))
        board_layout.append([sg.Text('    0              1              2             3             4             5             6             7' )])
        #print(board_layout)
        board_controls = [
            ...
            [sg.Button('Get Sample Voice'), sg.Text('Instruction(for Get Voice): Say four one digit nos.', key = '-listen-')]
        ]
    ...
    def main_loop(self):
        ...
        while True:
            ...
            if button == 'Get Sample Voice':
                #window.FindElement('-listen-').Update('Listening! Say 4 digit number ...')   ##########################################################################################
                print('Listening! Say any 4 digit number')
                '''Editing Here'''
                r = sr.Recognizer()
                with sr.Microphone() as source:
                    valid_input = True
                    try:
                        audio_data = r.listen(source)
                        print('Recognising...')
                        #window.FindElement('-listen-').Update('Recognising ...')
                        ...
                        #window.FindElement('-listen-').Update('Did not understand ...')
                        #valid_input = False
                        continue
                with sr.AudioFile('./speech.wav') as source:
                    try:
                        ...
                        for i in text:
                            if i not in checker:
                                print('Invalid Input')
                            #window.FindElement('-listen-').Update('Invalid Input ...')
                                validator = False
                                break
                        if (len(text)!=4) and validator:
                            print('Invalid Input')
                        #window.FindElement('-listen-').Update('Invalid Input ...')
                            validator = False
                        if validator :
                            temp = preprocess_wav('./speech.wav')
                            encoder = VoiceEncoder()
                            self.default_voice = encoder.embed_utterance(temp)
                    except:
                        validator = False
                        language = 'en'
                        output = gTTS(text="I didn't get that. Say again", lang=language, slow=False)
                    ...
                    #window.FindElement('-listen-').Update('Voice Updated successfully ...')
